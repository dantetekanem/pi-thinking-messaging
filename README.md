...
A small Pi extension that keeps Pi's original animated working loader, but adds elapsed time and a cumulative current-run token count to the active working message:

```text
Working… (7m 7s · ↓ 30.0k tokens)
```

The token number is always the cumulative current-run total (`input + output`) so it keeps increasing during a run. The token arrow shows the current movement direction: `↑` while request/input tokens are being sent upstream, and `↓` while assistant output tokens are coming back from the model.
